...
from telegram.request import _requestparameter
...
#     api_token = '6574457004:AAF2CQs75w8jYOcLDawmq1X5h-CVFXGAsoc'
#     chat_id = '-1001819419746'
#     url = f'https://api.telegram.org/bot{api_token}/sendMessage?chat_id={chat_id}&text={data}'
#     try:
...
#                 read_timeout=1.0,
#             )
#             bot = Updater(api_token, use_context=True, request=request)
#             dp = bot.dispatcher
#             dp.bot.send_message(chat_id=chat_id, text=message_text)
#             return JsonResponse({'status': 'success'})
#         else:
#             return JsonResponse({'status': 'error', 'message': 'Message is required'}, status=400)
#     except Exception as e:
#         logging.error(f'Error forwarding message: {e}')
#         return JsonResponse({'status': 'error', 'message': 'Error forwarding message'}, status=500)
...
def s(request):
    ...
    print(x)
    return Response({'status': 'success', 'message': x}, status=200)
# @api_view(['GET'])
...
#
#         time_stamp = str(time.time() * 1000 - 20000)
#         print('time1 ', time.time() * 1000)
